#!/usr/bin/env node
/**
 * Ensures each HTML page that loads styles.css?v=… has a matching
...
const STYLESHEET_RE =
  /<link\s+[^>]*\brel=["']stylesheet["'][^>]*\bhref=["']([^"']*styles\.css\?v=[^"']+)["'][^>]*>/i;
...
for (const f of walkHtml(root)) {
  if (patchFile(f)) {
    console.log('sync-styles-preload:', path.relative(root, f));
    n++;
  }
}
console.log(`sync-styles-preload: OK (${n} file(s) updated)`);
