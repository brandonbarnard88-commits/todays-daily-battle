...
- **Monetization (where-support-goes.html):** Cost table with specific ranges removed per direct user request ("WTF THATS RUDE AS FUCK TO DO THAT REMOVE IT"). Reverted to prior state listing priorities only (hosting, security, KJV upkeep, new plans/printables; no salaries). No dollar figures.
...
**Bottom line facts (no padding):** Heart (KJV-only, privacy-first, offline-first, practical plans), security (RLS, sanitization, no secrets in client), and core UX preserved. Explore bloat reduced, privacy wording hardened to explicit 90-day retention. Monetization table removed per user request. Site is functional solo v0.95. Visibility, language expectation gap, GA4 dependency, content memorability, solo succession note, and residual polish remain. All changes reviewable, no rules violated, no features removed, no regressions.
