...
**Shipped mitigations (2026-03-20, reader CLS — pane height):** `#chapter-reader #reader-output:not(.reader-output-empty)` uses **min-height matching** the existing `max-height` at each breakpoint (420px desktop, `50vh` ≤768px, 320px ≤480px) so the scroll pane does not jump from short skeleton/placeholder to the capped reading area when verses load. Empty state (`.reader-output-empty`) unchanged.
