...
- **Public write-only helpers** — `feeling_suggestions`, `contact_messages`, and `shop_waitlist` allow **anon INSERT only** (no anon SELECT), with length checks in RLS. Run `supabase-feeling-suggestions.sql`, `supabase-contact-messages.sql`, and `supabase-shop-waitlist.sql` in Supabase. Review rows in Dashboard or via `service_role`.
...
- **Headers** — `_headers` (Netlify/Cloudflare Pages): `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`, `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`, `Permissions-Policy: geolocation=(self), microphone=(), camera=(), payment=(), usb=(), battery=()` (optional same-origin geolocation is used only on the homepage for local sunrise/sunset sky styling; coordinates are not sent to our servers—see `index.html` sky script), `X-XSS-Protection: 1; mode=block`.
