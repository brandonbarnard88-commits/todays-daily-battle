...
- **RLS on all synced tables** — `user_sync_data`, `messages`, `message_reports`, `newsletter_signups`, `prayers`, `daily_battles`, `team_codes`, `team_verse_of_day`, `team_sync_data` (and church_* tables). Policies restrict access by `auth.uid()` or role; new team tables use service_role + SECURITY DEFINER RPCs only (modeled on church_groups). See `supabase-rls-lockdown.sql`, `supabase-rls-quick.sql`, and `supabase-team-sync.sql`.
...
- [ ] **New Supabase table** — Enable RLS; add policies so only intended roles (e.g. `authenticated`, or anon only for specific actions) can read/write. For team features use service_role + SECURITY DEFINER RPCs (see `supabase-team-sync.sql`, modeled on `supabase-church-groups.sql`). Prefer `auth.uid() = user_id` for per-user data. Always update SUPABASE-SYNC-TABLES.md and this checklist.
