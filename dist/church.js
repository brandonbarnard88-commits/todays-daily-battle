(function () {
  ...
  async function officialTodayVerse() {
    try {
      var res = await fetch('/today-kjv-verse.json', { cache: 'no-store' });
      if (res.ok) {
        var row = await res.json();
        if (row && row.ref && row.text) return { ref: String(row.ref), text: String(row.text) };
      }
  ...
  async function renderDaily(code) {
    try {
      var v = await officialTodayVerse();
      if (!v) v = await dailyVerseForCode(code);
