/**
 * Bundled daily verses for offline/fallback use. getDailyVerse() picks by date.
 * Use when kjv.json fetch fails or for service worker offline.
 */
(function (root) {
  var dailyVerses = [
  ...
  function getDailyVerse() {
    var today = new Date().getDate(); // 1–31
    var idx = (today - 1) % dailyVerses.length;
    return dailyVerses[idx] || dailyVerses[0];
  }

  root.DAILY_VERSES = dailyVerses;
  root.getDailyVerse = getDailyVerse;
