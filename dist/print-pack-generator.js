  ...
  function copyCurrentLink() {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () {
        ...
        if (status) status.textContent = 'Copy did not go through. You can copy the URL from the address bar.';
      });
      return;
    }
    if (status) status.textContent = 'Copy is not available in this browser. You can copy the URL from the address bar.';
  }
