...
const CACHE_NAME = 'tdb-cache-v20260905porchsky1';
...
const CORE_ASSETS = [
  ...
  '/data/kjv-full.json',
  /* BBE stays on-demand via bbe-simple.js — do not precache 4.5MB (and a missing file 404s every SW install). */
  '/bbe-simple.js',
...
self.addEventListener('fetch', (event) => {
  ...
  /* Kids hub HTML must be network-first so “Today is Glad day.” cannot stick in precache. */
  if (
    event.request.mode === 'navigate' &&
    (url.pathname === '/kids' || url.pathname === '/kids/' || url.pathname === '/kids/index.html')
  ) {
    event.respondWith(
      fetch(event.request)
        .then(function (res) {
          if (res && res.ok && !res.redirected) {
            ...
            caches.open(CACHE_NAME).then(function (cache) {
              cache.put(event.request, clone).catch(function () {});
              try {
                cache.put('/kids/index.html', res.clone()).catch(function () {});
              } catch (_eKids) {}
            });
          }
          return res;
