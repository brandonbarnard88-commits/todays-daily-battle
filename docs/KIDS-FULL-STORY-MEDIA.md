...
**Read + quiz (no video required):** `kids/kids-read-quiz-data.js` defines `TDB_KIDS_READ_QUIZ` for **every** `bibleStories` key (currently **172**), wired on `/kids/corner.html` in the story modal.
