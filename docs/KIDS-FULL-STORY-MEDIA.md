...
- **Hand-tuned** packs (higher-quality copy + quizzes): edit `kids/read-quiz-handcrafted.cjs` (e.g. `david`, `noah`, `jonah`, `daniel`, `fallOfJericho` + `jerichoWalls`, `jesusCalmsStorm`, `goodSamaritan`, `lostSheep`), then run the same command so the big file picks them up.
