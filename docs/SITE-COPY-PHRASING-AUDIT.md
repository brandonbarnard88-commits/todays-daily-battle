...
## Implementation status (2026-08-11 — `fix/site-copy-all`)

| Phase | Status |
|-------|--------|
| 1 Naming + FAQ + dig-deeper + search + plans strip | Done |
| 2 Dual doors (kids surprise, match labels, plan near-dupes) | Done |
| 3 Tone bulk (“that is all right”, welcome mantras, My Study) | Done (majority) |
| 4 Catalog titles + phrase lint | Done (key titles + `verify:site-copy`) |

Canonical names applied across ~250 user-facing HTML/JS files. Lint: `npm run verify:site-copy`.
