...
for (const rel of FR_FOOTER_FILES) {
  const ok = patchFile(rel, (html) => {
    if (!html.includes('footer-sitemap')) return html;
    return html.replace(FOOTER_RE, frFooter());
  });
  if (ok) {
    changed++;
...
for (const rel of PT_FOOTER_FILES) {
  const ok = patchFile(rel, (html) => {
    if (!html.includes('footer-sitemap')) return html;
    return html.replace(FOOTER_RE, ptFooter());
  });
  if (ok) {
    changed++;
...
// FR pilots: fix header switcher lang/aria when still EN
const FR_HEADER_BAD =
...
console.log('Done. Footers replaced in', changed, 'files (had footer-sitemap).');
