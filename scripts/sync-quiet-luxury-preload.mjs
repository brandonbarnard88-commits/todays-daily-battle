#!/usr/bin/env node
/**
 * Ensures each HTML page that loads tdb-quiet-luxury.css?v=… has a matching
...
const STYLESHEET_RE =
  /<link\s+[^>]*\brel=["']stylesheet["'][^>]*\bhref=["']([^"']*tdb-quiet-luxury\.css\?v=[^"']+)["'][^>]*>/i;
...
for (const f of walkHtml(root)) {
  if (patchFile(f)) {
    console.log('sync-quiet-luxury-preload:', path.relative(root, f));
    n++;
  }
}
console.log(`sync-quiet-luxury-preload: OK (${n} file(s) updated)`);
